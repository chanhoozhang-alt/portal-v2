...
**用户不会在本系统登录。** 用户在外部统一门户登录后，点击"南京分行应用管理系统"卡片，带着 Token 跳转进来。
...
用户从外部统一门户跳转进来时，URL 上会携带 Token 参数，例如：

```
https://your-domain.com/?token=eyJhbGciOiJIUzI1NiJ9...
```

### 3.2 Token 使用方式

前端拿到 Token 后，所有接口请求都要在 Header 中携带：
...
- Token 由外部统一身份平台签发，有效期由平台控制
- 后端会将用户身份缓存到 Redis（TTL 30 分钟），每次请求自动续期
- Token 无效或过期时，后端返回 `401`，前端应引导用户回到外部门户重新登录

### 3.4 前端处理流程

```
1. 用户从外部门户跳转进来，URL 带有 token 参数
2. 前端从 URL 解析 token，存入 localStorage 或内存
3. 调用 GET /api/portal/init 获取用户信息和页面数据
4. 后续所有请求在 Header 中带上 Authorization: Bearer {token}
5. 如果返回 401，清除本地 token，跳回外部门户
```
...
3. 用户点击应用卡片 → 调用 GET /api/portal/apps/{appCode}/jump-info
4. 拿到 jumpUrl 后跳转（iframe 或新窗口，取决于 showMenu/showHeader 配置）
```
...
```
401 → 清除 token，跳转回外部门户
403 → 提示"无权限"，停留在当前页或返回上一页
其他 → 显示后端返回的 message
```
