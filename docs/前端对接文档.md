...
### 3.1 当前认证方式（2026-05-18 更新）

当前已接入上游授权码登录模式。本系统作为“南京分行应用管理系统”注册在上游统一认证平台中，前端不再从 URL 中解析上游 Token，也不保存 `accessToken` / `refreshToken` / `id_token`。
...
当前项目是新系统，正式链路只要求浏览器自动携带 `PORTAL_SESSION` Cookie；前端不要手动拼接 `Authorization` 请求头。
...
flowchart TD
    ...
    H --> I["后端换 token<br/>解析 id_token<br/>建立 PORTAL_SESSION"]
    I --> J["302 回门户首页 /"]
    J --> B
```
...
6. 后端换取 access_token / refresh_token / id_token，解析 id_token 后建立 PORTAL_SESSION，并 302 回 redirect 页面
...
- 不要保存上游 `accessToken` / `refreshToken` / `id_token` 到 `localStorage` 或 `sessionStorage`。
...
- 后端保存上游 token，并用 `PORTAL_SESSION` 关联 Redis 会话；用户身份以后端解析 `id_token` 为准。
...
所有后台接口都需要已登录态。浏览器自动携带 `PORTAL_SESSION` Cookie；前端不要手动传 `Authorization: Bearer {token}`。
...
启停:   PUT  /api/admin/apps/{appCode}/custom-roles/users/status  body: {"roleCode":"op","userIds":["U001","U002"],"status":"ENABLED"}
...
前端首页 /
    │
    │ 1. GET /api/portal/init
    │ 2. 未登录时跳 /api/auth/login?redirect=/
    │ 3. 登录成功后浏览器自动携带 PORTAL_SESSION
    ▼
首页渲染
    │
...
| 项目 | 上一版本 | 当前版本 | 前端需要做什么 |
| --- | --- | --- | --- |
| 用户身份来源 | 后端可能通过 `accessToken` 回源校验用户身份 | 后端只解析上游 `id_token` 获取用户身份 | 前端无感知，不要处理 `id_token` |
| 请求认证方式 | 文档曾说明后端兼容 `Authorization: Bearer {token}` | 正式链路只使用 `PORTAL_SESSION` Cookie | 删除前端统一加 `Authorization` 的逻辑 |
| token 保存 | 不保存 `accessToken` / `refreshToken` | 也不保存 `id_token` | 检查 `localStorage` / `sessionStorage` / Pinia/Vuex/Redux 中不要存任何上游 token |
| portal 接口 | 浏览器携带 Cookie 或手动 Header 均可能可用 | 只要求浏览器自动携带 Cookie | AJAX 请求保持同源或开启携带 Cookie，不拼 token |
| admin 接口 | 文档曾说明兼容 Bearer Token | 后台接口同样只依赖 `PORTAL_SESSION` Cookie | 后台请求也不要传 `Authorization` |
| 401 处理 | 401 跳 `/api/auth/login?redirect=当前页面` | 不变 | 保留当前 401 统一跳转逻辑 |
| 403 处理 | 后台越权展示无权限 | 不变 | 不要把 403 当 401 重新登录 |
| 登录回调 | 后端换 token 后建 session | 后端换 `access_token` / `refresh_token` / `id_token`，解析后建 session | 前端仍不处理 callback 参数 |

前端改造检查清单：

- 全局请求拦截器不要再从本地状态读取 token 并写入 `Authorization`。
- 不要新增任何读取、保存、解析 `id_token` 的逻辑。
- 跨域开发代理如果不是同源，需要确保请求携带 Cookie；生产推荐同源反代。
- `GET /api/portal/init` 和后台 `/api/admin/**` 的 401 处理保持统一跳登录。
- 退出登录仍调用 `POST /api/auth/logout`，不要只清前端状态。
