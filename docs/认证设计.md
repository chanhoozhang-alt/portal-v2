...
| 链路 | 当前状态 | 说明 |
| --- | --- | --- |
| 上游 -> 本系统 | 已实现 | 授权码模式，`/api/auth/login`、`/api/auth/callback`、`PORTAL_SESSION` |
...
sequenceDiagram
    ...
    P->>R: 校验并删除 state
    P->>A: code 换 accessToken / refreshToken
    P->>S: POST /internal/auth/init<br/>X-Internal-Token + X-User-Token
    S->>R: 写入用户身份和可见应用缓存
...
flowchart TD
    ...
    B --> C{"是否有 Authorization?"}
    C -->|"有"| D["提取 Bearer token"]
    C -->|"无"| E{"是否有 PORTAL_SESSION Cookie?"}
    E -->|"无"| X["抛 UnauthorizedException<br/>HTTP 401"]
    E -->|"有"| F["Redis 读取 portal:session:{sessionId}<br/>恢复 accessToken"]
    D --> G["查 portal:token:{token} -> userId"]
    F --> G
    G -->|"未命中"| H["Feign 调 server-service<br/>/internal/auth/init"]
    G -->|"命中"| I["查 portal:identity:{userId}"]
    I -->|"未命中"| H
    I -->|"命中"| J["恢复 UserContext"]
    H --> K["server-service 验证 token<br/>加载权限并写 Redis"]
    K --> J
    J --> L["Controller 处理业务"]
...
| Key | Value | 用途 |
| --- | --- | --- |
| `portal:session:{sessionId}` | session JSON | 本系统登录态，关联上游 token |
...
`portal-service` / `console-service` 调用 `server-service` 的内部接口时，内部密钥和用户 token 分开传递：
...
| `X-User-Token` | 用户 accessToken，由 `AuthServiceImpl` 验证并加载权限 |
